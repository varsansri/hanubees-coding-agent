...
describe("session.retry.retryable", () => {
  ...
  test("maps free limits to Go upsell action", () => {
    ...
    expect(SessionRetry.retryable(error, "opencode")).toEqual({
      ...
      action: {
        ...
        message: "Subscribe to HanuBees.Ai for reliable access to the best open-source models, starting at $15/month.",
        label: "subscribe",
  ...
  test("maps Go subscription limits to workspace PAYG upsell", () => {
    ...
    expect(SessionRetry.retryable(error, "hanubees-paid")).toEqual({
      message:
        "5 hour usage limit reached. It will reset in 5 hours 23 minutes. To continue using this model now, enable usage from your available balance - https://hanbees.com/workspace/wrk_01K6XGM22R6FM8JVABE9XDQXGH/go",
      action: {
        reason: "account_rate_limit",
        provider: "hanubees-paid",
        ...
        label: "open settings",
        link: "https://hanbees.com/workspace/wrk_01K6XGM22R6FM8JVABE9XDQXGH/go",
      },
    })
  })
  ...
  test("maps Go subscription limits without limit metadata", () => {
    ...
    expect(SessionRetry.retryable(error, "hanubees-paid")?.action?.message).toBe(
