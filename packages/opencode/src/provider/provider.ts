  ...
  return {
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `hanubees auth cloudflare-ai-gateway`.",
        )
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "HanuBees",
          },
        },
      }),
    kilo: () =>
...
const priority = ["gpt-5", "claude-sonnet-4", "hanubees-ai", "gemini-3-pro"]
