This is a plugin to simulate a remote environment locally. Add this to `.HanuBees/HanuBees.jsonc`:

```json
  "plugin": ["../packages/HanuBees/src/control-plane/dev/debug-workspace-plugin.ts"],
```

In a separate terminal, run a separate HanuBees server. This will act like a remote server and the local instance will proxy all requests to it:

```
./packages/HanuBees/script/run-workspace-server
```

With the plugin install, you can now run HanuBees and create a `debug` workspace type. This will create a "remote" workspace which talks to the second workspace server started above.
