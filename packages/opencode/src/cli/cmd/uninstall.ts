...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall HanuBees and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall HanuBees")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g hanubees-ai",
      pnpm: "pnpm uninstall -g hanubees-ai",
      bun: "bun remove -g hanubees-ai",
      yarn: "yarn global remove hanubees-ai",
      brew: "brew uninstall hanubees",
      choco: "choco uninstall hanubees",
      scoop: "scoop uninstall hanubees",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "hanubees", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using HanuBees!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# hanubees") || content.includes(".hanubees/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".hanubees/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".hanubees/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".hanubees"))
    ) {
      continue
    }
