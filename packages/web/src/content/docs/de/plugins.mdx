---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um HanuBees zu erweitern.
---

Mit Plugins können Sie HanuBees erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von HanuBees zu ändern.
...
- `.HanuBees/plugins/` – Plugins auf Projektebene
- `~/.config/HanuBees/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/HanuBees/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/HanuBees/HanuBees.json`)
2. Projektkonfiguration (`HanuBees.json`)
3. Globales Plugin-Verzeichnis (`~/.config/HanuBees/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.HanuBees/plugins/`)
...
HanuBees führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein HanuBees-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie HanuBees erweitern können.
...
:::note
Wenn Sie die HanuBees-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass HanuBees `.env`-Dateien liest:
...
Plugins können HanuBees auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das HanuBees aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für HanuBees zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://hanubees.com/docs/sdk).
