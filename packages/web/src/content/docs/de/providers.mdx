---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in HanuBees.
---

import config from "../../../../config.mjs"
export const console = config.console

HanuBees verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer HanuBees-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem HanuBees anpassen
config.
...
HanuBees Zen ist eine Liste von Modellen, die vom HanuBees-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit HanuBees funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit HanuBees Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie HanuBees aus und gehen Sie zu [hanubees.com/auth](https://hanubees.com/auth).
...
Es funktioniert wie jeder andere Anbieter in HanuBees und ist völlig optional.

---

## HanuBees Go

HanuBees Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom HanuBees-Team getestet und verifiziert wurden, dass sie gut mit HanuBees funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `HanuBees Go` und gehen Sie zu [hanubees.com/auth](https://hanubees.com/zen).
...
So verwenden Sie Amazon Bedrock mit HanuBees:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie HanuBees ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `HanuBees.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in HanuBees wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können HanuBees so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit HanuBees ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre HanuBees-Konfiguration hinzufügen.
...
um das Token im HanuBees-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
HanuBees verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von HanuBees
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`HanuBees.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [HanuBees-gitlab-auth](https://www.npmjs.com/package/HanuBees-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `HanuBees.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit HanuBees:
...
So verwenden Sie Google Vertex AI mit HanuBees:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie HanuBees ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über HanuBees konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [HanuBees-helicone-session](https://github.com/H2Shami/HanuBees-helicone-session)-Plugin, um jede HanuBees-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede HanuBees-Konversation als separate Sitzung aufgeführt.
...
Sie können HanuBees für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können HanuBees über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können HanuBees über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für HanuBees konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/HanuBees).
:::
...
So verwenden Sie Ollama Cloud mit HanuBees:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in HanuBees.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in HanuBees verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
HanuBees Zen ist eine Liste getesteter und verifizierter Modelle, die vom HanuBees-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>HanuBees Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **HanuBees Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren HanuBees API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre HanuBees-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre HanuBees-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit HanuBees:
...
Sie können Modelle auch über Ihre HanuBees-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit HanuBees verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `HanuBees.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann HanuBees erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `HanuBees auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die HanuBees-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer HanuBees-Konfiguration übereinstimmt.
