...
Mit HanuBees kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet HanuBees das `HanuBees`-Theme.
...
HanuBees liefert mehrere eingebaute Themes mit.
...
- HanuBees optisch ans Terminal anpassen wollen
...
HanuBees unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/HanuBees/themes/*.json` oder `$XDG_CONFIG_HOME/HanuBees/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.HanuBees/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.HanuBees/themes/*.json`
