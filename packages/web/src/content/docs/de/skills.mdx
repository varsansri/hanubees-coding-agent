...
Agent Skills erlauben HanuBees, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
HanuBees sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft HanuBees vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.HanuBees/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/HanuBees/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.HanuBees/skills/git-release/SKILL.md` zum Beispiel so:
...
HanuBees listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `HanuBees.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `HanuBees.json`):
