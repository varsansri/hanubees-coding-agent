...
Wenn HanuBees Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\HanuBees\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `HanuBees --log-level DEBUG`.

---

## Speicher

HanuBees speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/HanuBees/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\HanuBees`
...
HanuBees Desktop startet im Hintergrund einen lokalen HanuBees-Server (`HanuBees-cli`-Sidecar).
...
- Nur macOS: `HanuBees`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/HanuBees/HanuBees.jsonc` (oder `~/.config/HanuBees/HanuBees.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/HanuBees/HanuBees.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\HanuBees\HanuBees.jsonc`
...
HanuBees kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/HanuBees/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\HanuBees\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit HanuBees ihn neu aufbauen kann.

1. Beende HanuBees Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/HanuBees`
- **Linux**: loesche `~/.cache/HanuBees` (oder fuehre aus: `rm -rf ~/.cache/HanuBees`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\HanuBees`

3. Starte HanuBees Desktop neu.

---

### Server-Verbindungsprobleme beheben

HanuBees Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `HanuBees.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `HanuBees_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `HanuBees_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt HanuBees Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit HanuBees-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

HanuBees Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer HanuBees in den OS-Einstellungen aktiviert sind, und
...
1. Beende HanuBees Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von HanuBees Desktop):

- `HanuBees.settings.dat` (desktop default server URL)
- `HanuBees.global.dat` und `HanuBees.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit HanuBees hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `HanuBees models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\HanuBees`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. HanuBees installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\HanuBees`

2. Starte HanuBees neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt HanuBees, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
HanuBees erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
