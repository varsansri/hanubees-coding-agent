...
description: Erstellen Sie Tools, die der LLM in HanuBees aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von HanuBees wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.HanuBees/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/HanuBees/tools/` platzieren.
