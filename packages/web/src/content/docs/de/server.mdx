---
title: Server
description: Nutze den HanuBees-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `HanuBees serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein HanuBees-Client nutzen kann.
...
Setze `HanuBees_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `HanuBees`, kann aber mit `HanuBees_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `HanuBees serve` und `HanuBees web`.
...
Wenn du `HanuBees` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den HanuBees-Server, um HanuBees programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `HanuBees serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `HanuBees serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die HanuBees-[IDE](/docs/ide)-Plugins.
...
Der HanuBees-Server stellt folgende APIs bereit.
