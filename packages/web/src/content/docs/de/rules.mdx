---
title: Regeln
description: Lege eigene Anweisungen fuer HanuBees fest.
---

Du kannst HanuBees ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in HanuBees den Befehl `/init` aus.
...
So kann HanuBees sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht HanuBees sie zu erweitern.
...
HanuBees kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/HanuBees/AGENTS.md` speichern.
Diese gelten in allen HanuBees-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt HanuBees dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/HanuBees/AGENTS.md` existiert)
...
Beim Start sucht HanuBees in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/HanuBees/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `HanuBees.json` oder in `~/.config/HanuBees/HanuBees.json` angeben.
...
HanuBees parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit HanuBees.json

Empfohlen ist das `instructions`-Feld in der `HanuBees.json`:
...
Du kannst HanuBees explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `HanuBees.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
