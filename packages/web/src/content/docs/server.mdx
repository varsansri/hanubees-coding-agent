---
title: Server
description: Interact with HanuBees server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `HanuBees serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an HanuBees client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `HanuBees.local` |
...
Set `HanuBees_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `HanuBees`, or set `HanuBees_SERVER_USERNAME` to override it. This applies to both `HanuBees serve` and `HanuBees web`.
...
When you run `HanuBees` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the HanuBees server to interact with HanuBees programmatically.
:::

This architecture lets HanuBees support multiple clients and allows you to interact with HanuBees programmatically.

You can run `HanuBees serve` to start a standalone server. If you have the
HanuBees TUI running, `HanuBees serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the HanuBees [IDE](/docs/ide) plugins.
...
The HanuBees server exposes the following APIs.
