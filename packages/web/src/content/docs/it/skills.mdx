...
Le skill degli agenti permettono a HanuBees di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
HanuBees cerca in queste posizioni:

- Config di progetto: `.HanuBees/skills/<name>/SKILL.md`
- Config globale: `~/.config/HanuBees/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, HanuBees risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.HanuBees/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/HanuBees/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.HanuBees/skills/git-release/SKILL.md` cosi':
...
HanuBees elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `HanuBees.json`:
...
**Per agenti integrati** (in `HanuBees.json`):
