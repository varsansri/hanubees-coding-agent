---
title: Plugin
description: Scrivi plugin per estendere HanuBees.
---

I plugin ti permettono di estendere HanuBees agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di HanuBees.
...
- `.HanuBees/plugins/` - plugin a livello progetto
- `~/.config/HanuBees/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/HanuBees/node_modules/`.
...
1. Config globale (`~/.config/HanuBees/HanuBees.json`)
2. Config di progetto (`HanuBees.json`)
3. Directory plugin globale (`~/.config/HanuBees/plugins/`)
4. Directory plugin di progetto (`.HanuBees/plugins/`)
...
HanuBees esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di HanuBees per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere HanuBees.
...
:::note
Se usi l'app desktop di HanuBees, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a HanuBees di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a HanuBees:
...
L'helper `tool` crea uno strumento personalizzato che HanuBees può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in HanuBees insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://hanubees.com/docs/sdk) per i dettagli.
