---
title: Server
description: Interagisci con il server HanuBees via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `HanuBees serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client HanuBees.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `HanuBees.local` |
...
Imposta `HanuBees_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `HanuBees`, oppure imposta `HanuBees_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `HanuBees serve` sia per `HanuBees web`.
...
Quando esegui `HanuBees` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server HanuBees per interagire con HanuBees in modo programmatico.
:::

Questa architettura permette a HanuBees di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `HanuBees serve` per avviare un server standalone. Se la TUI di HanuBees e gia in esecuzione, `HanuBees serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di HanuBees.
...
Il server HanuBees espone le seguenti API.
