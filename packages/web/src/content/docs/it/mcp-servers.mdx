...
Puoi aggiungere strumenti esterni a HanuBees usando il _Model Context Protocol_, o MCP. HanuBees supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [HanuBees Config](https://hanubees.com/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/HanuBees`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
HanuBees gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, HanuBees:
...
Se il server richiede autenticazione, HanuBees ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `HanuBees mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, HanuBees memorizzera' i token in modo sicuro in `~/.local/share/HanuBees/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in HanuBees insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione HanuBees come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare HanuBees al tuo account Sentry.
