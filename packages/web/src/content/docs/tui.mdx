---
title: TUI
description: Using the HanuBees terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

HanuBees provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running HanuBees starts the TUI for the current directory.
...
When using the HanuBees TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to HanuBees. Allows you to select from available providers and add their API keys.
...
Exit HanuBees. _Aliases_: `/quit`, `/q`
...
This is separate from `HanuBees.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long HanuBees waits after the leader key. Defaults to `2000`.
...
Use `HanuBees_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `HanuBees.default`.
