---
title: Providers
description: Using any LLM provider in HanuBees.
---

import config from "../../../config.mjs"
export const console = config.console

HanuBees uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your HanuBees config.
...
in `~/.local/share/HanuBees/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your HanuBees
config.
...
HanuBees Zen is a list of models provided by the HanuBees team that have been
tested and verified to work well with HanuBees. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with HanuBees Zen.
:::

1. Run the `/connect` command in the TUI, select `HanuBees Zen`, and head to [hanubees.com/auth](https://hanubees.com/zen).
...
It works like any other provider in HanuBees and is completely optional to use.

---

## HanuBees Go

HanuBees Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the HanuBees team that have been
tested and verified to work well with HanuBees.

1. Run the `/connect` command in the TUI, select `HanuBees Go`, and head to [hanubees.com/auth](https://hanubees.com/zen).
...
To use Amazon Bedrock with HanuBees:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running HanuBees:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY HanuBees

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile HanuBees

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX HanuBees
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `HanuBees.json`:
...
HanuBees. Anthropic explicitly prohibits this.

Previous versions of HanuBees came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in HanuBees with zero setup:
...
You can configure HanuBees to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="HanuBees.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://hanubees.com/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for HanuBees to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX HanuBees
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX HanuBees
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your HanuBees config.
...
HanuBees supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; HanuBees uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize HanuBees. Sign in and approve.

   :::note
   HanuBees requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, HanuBees surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — HanuBees forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in HanuBees is experimental. Features, configuration, and
behavior may change in future releases.
:::

HanuBees integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in HanuBees auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
HanuBees uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock HanuBees
to only use your own GitLab-hosted instance, add the following to your
`HanuBees.json` file. It is also recommended to disable session sharing.
...
More documentation on [HanuBees-gitlab-auth](https://www.npmjs.com/package/HanuBees-gitlab-auth) homepage.

##### Configuration

Customize through `HanuBees.json`:
...
When a `duo-workflow-*` model is selected, HanuBees will:
...
4. Route tool execution requests through HanuBees's permission-gated tool system
...
To use your GitHub Copilot subscription with HanuBees:
...
To use GMI Cloud with HanuBees:
...
To use Google Vertex AI with HanuBees:
...
2. Set the required environment variables:
   ...
   Set them while running HanuBees.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id HanuBees
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through HanuBees, you can always configure it yourself.
...
```jsonc title="~/.config/HanuBees/HanuBees.jsonc"
{
  "$schema": "https://hanubees.com/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [HanuBees-helicone-session](https://github.com/H2Shami/HanuBees-helicone-session) plugin to automatically log each HanuBees conversation as a session in Helicone.

```bash
npm install -g hanubees-helicone-session
```
...
```json title="HanuBees.json"
{
  "plugin": ["HanuBees-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each HanuBees conversation listed as a separate session.
...
You can configure HanuBees to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="HanuBees.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://hanubees.com/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure HanuBees to use local models through LM Studio.

```json title="HanuBees.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://hanubees.com/config.json",
  "provider": {
    "lmstudio": {
...
You can configure HanuBees to use local models through Ollama.

:::tip
Ollama can automatically configure itself for HanuBees. See the [Ollama integration docs](https://docs.ollama.com/integrations/HanuBees) for details.
:::

```json title="HanuBees.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://hanubees.com/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with HanuBees:
...
3. Copy the API key for use in HanuBees.
...
6. **Important**: Before using cloud models in HanuBees, you must pull the model information locally:
...
HanuBees Zen is a list of tested and verified models provided by the HanuBees team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>HanuBees Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **HanuBees Zen**.

   ```txt
   /connect
   ```

3. Enter your HanuBees API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your HanuBees config.
...
5. You can also customize them through your HanuBees config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' HanuBees
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group HanuBees
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with HanuBees:
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=HanuBees&utm_medium=docs&utm_campaign=cortex-provider).

HanuBees's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. HanuBees captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting HanuBees:
...
The model catalog is provided automatically. A minimal `HanuBees.json` is all that's needed:
...
You can also customize models through your HanuBees config. Here's an example of specifying provider routing order.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. HanuBees opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
HanuBees refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when HanuBees is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running HanuBees — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. HanuBees prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. HanuBees polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with HanuBees. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in HanuBees.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `HanuBees.json` file in your project directory:

   ```json title="HanuBees.json" ""myprovider"" {5-15}
   {
     "$schema": "https://hanubees.com/config.json",
     "provider": {
       "myprovider": {
...
```json title="HanuBees.json" {9,11,17-20}
{
  "$schema": "https://hanubees.com/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow HanuBees to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `HanuBees auth list` to see if the credentials
...
2. For custom providers, check the HanuBees config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your HanuBees config.
