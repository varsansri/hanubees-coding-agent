...
Create `.HanuBees/commands/test.md`:
...
You can add custom commands through the HanuBees config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your HanuBees [config](/docs/config):
...
HanuBees includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
