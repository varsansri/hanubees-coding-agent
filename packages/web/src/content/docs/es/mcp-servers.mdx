...
Puede agregar herramientas externas a HanuBees utilizando el _Model Context Protocol_ o MCP. HanuBees admite servidores locales y remotos.
...
Puede definir servidores MCP en su [HanuBees Config](https://hanubees.com/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/HanuBees`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
HanuBees maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, HanuBees:
...
Si el servidor requiere autenticación, HanuBees le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `HanuBees mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, HanuBees almacenará los tokens de forma segura en `~/.local/share/HanuBees/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en HanuBees, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración HanuBees como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar HanuBees a su cuenta Sentry.
