...
Las habilidades del agente permiten a HanuBees descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
HanuBees busca estas ubicaciones:

- Configuración del proyecto: `.HanuBees/skills/<name>/SKILL.md`
- Configuración global: `~/.config/HanuBees/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, HanuBees sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.HanuBees/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/HanuBees/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.HanuBees/skills/git-release/SKILL.md` así:
...
HanuBees enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `HanuBees.json`:
...
**Para agentes integrados** (en `HanuBees.json`):
