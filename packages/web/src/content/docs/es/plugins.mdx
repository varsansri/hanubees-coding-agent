---
title: Complementos
description: Escriba sus propios complementos para extender HanuBees.
---

Los complementos le permiten extender HanuBees al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de HanuBees.
...
- `.HanuBees/plugins/` - Complementos a nivel de proyecto
- `~/.config/HanuBees/plugins/` - Complementos globales
...
```json title="HanuBees.json"
{
  ...
  "plugin": ["HanuBees-helicone-session", "HanuBees-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/HanuBees/node_modules/`.
...
1. Configuración global (`~/.config/HanuBees/HanuBees.json`)
2. Configuración del proyecto (`HanuBees.json`)
3. Directorio global de complementos (`~/.config/HanuBees/plugins/`)
4. Directorio de complementos del proyecto (`.HanuBees/plugins/`)
...
```json title=".HanuBees/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
HanuBees ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".HanuBees/plugins/my-plugin.ts"
...
```js title=".HanuBees/plugins/example.js"
...
- `client`: Un cliente SDK HanuBees para interactuar con la IA.
...
import type { Plugin } from "@hanubees/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar HanuBees.
...
```js title=".HanuBees/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "HanuBees"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio HanuBees, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que HanuBees lea archivos `.env`:

```javascript title=".HanuBees/plugins/env-protection.js"
...
```javascript title=".HanuBees/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a HanuBees:

```ts title=".HanuBees/plugins/custom-tools.ts"
import { type Plugin, tool } from "@hanubees/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que HanuBees puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para HanuBees junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://hanubees.com/docs/sdk) para obtener más detalles.
...
```ts title=".HanuBees/plugins/compaction.ts"
import type { Plugin } from "@hanubees/plugin"
...
```ts title=".HanuBees/plugins/custom-compaction.ts"
import type { Plugin } from "@hanubees/plugin"
