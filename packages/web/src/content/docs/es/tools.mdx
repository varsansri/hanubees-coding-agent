...
Las herramientas permiten que LLM realice acciones en su código base. HanuBees viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en HanuBees.
...
:::note
Esta herramienta solo está disponible cuando `HanuBees_EXPERIMENTAL_LSP_TOOL=true` (o `HanuBees_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor HanuBees o cuando la variable de entorno `HanuBees_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar HanuBees:

```bash
HanuBees_ENABLE_EXA=1 HanuBees
```
