---
title: Servidor
description: Interactuar con el servidor HanuBees a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `HanuBees serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente HanuBees puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `HanuBees.local` |
...
Configure `HanuBees_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `HanuBees`, o configure `HanuBees_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `HanuBees serve` como a `HanuBees web`.
...
Cuando ejecuta `HanuBees`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor HanuBees para interactuar con HanuBees mediante programación.
:::

Esta arquitectura permite que HanuBees admita múltiples clientes y le permite interactuar con HanuBees mediante programación.

Puede ejecutar `HanuBees serve` para iniciar un servidor independiente. Si tienes el
HanuBees TUI ejecutándose, `HanuBees serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos HanuBees [IDE](/docs/ide).
...
El servidor HanuBees expone las siguientes API.
