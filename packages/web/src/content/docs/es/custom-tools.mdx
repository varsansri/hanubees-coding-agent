...
description: Cree herramientas que LLM pueda llamar en HanuBees.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de HanuBees como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.HanuBees/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/HanuBees/tools/`.
...
```ts title=".HanuBees/tools/database.ts" {1}
import { tool } from "@hanubees/plugin"
...
```ts title=".HanuBees/tools/math.ts"
import { tool } from "@hanubees/plugin"
...
```ts title=".HanuBees/tools/bash.ts"
import { tool } from "@hanubees/plugin"
...
```ts title=".HanuBees/tools/project.ts" {8}
import { tool } from "@hanubees/plugin"
...
```python title=".HanuBees/tools/add.py"
import sys
...
```ts title=".HanuBees/tools/python-add.ts" {10}
import { tool } from "@hanubees/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".HanuBees/tools/add.py")
