---
title: サーバー
description: HTTP 経由で HanuBees サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`HanuBees serve` コマンドは、HanuBees クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `HanuBees.local` |
...
HTTP 基本認証でサーバーを保護するには、`HanuBees_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `HanuBees` になるか、`HanuBees_SERVER_USERNAME` を設定してオーバーライドします。これは、`HanuBees serve` と `HanuBees web` の両方に当てはまります。
...
`HanuBees` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
HanuBees サーバーを使用して、プログラムで HanuBees と対話します。
:::

`HanuBees serve` を実行してスタンドアロンサーバーを起動できます。すでに HanuBees TUI を実行している場合でも、`HanuBees serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、HanuBees [IDE](/docs/ide) プラグインによって使用されます。
