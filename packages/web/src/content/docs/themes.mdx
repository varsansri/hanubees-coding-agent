...
With HanuBees you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, HanuBees uses our own `HanuBees` theme.
...
HanuBees comes with several built-in themes.
...
- Want HanuBees to match their terminal's appearance
...
HanuBees supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/HanuBees/themes/*.json` or `$XDG_CONFIG_HOME/HanuBees/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.HanuBees/themes/*.json`
4. **Current working directory** - Defined in `./.HanuBees/themes/*.json`
