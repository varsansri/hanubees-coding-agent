---
title: Server
description: Interager med HanuBees-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `HanuBees serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en HanuBees-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `HanuBees.local` |
...
Indstil `HanuBees_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `HanuBees`, eller indstil `HanuBees_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `HanuBees serve` og `HanuBees web`.
...
Når du kører `HanuBees` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug HanuBees-serveren til at interagere med HanuBees programmatisk.
:::

Denne arkitektur lader HanuBees understøtte flere klienter og lader dig interagere med HanuBees programmatisk.

Du kan køre `HanuBees serve` for at starte en selvstændig server. Hvis du har
HanuBees TUI kørende, vil `HanuBees serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af HanuBees [IDE](/docs/ide) plugins.
...
HanuBees-serveren viser følgende API'er.
