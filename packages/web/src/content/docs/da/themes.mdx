...
Med HanuBees kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger HanuBees vårt eget `HanuBees`-tema.
...
HanuBees kommer med flere innebygde temaer.
...
- Vil at HanuBees skal matche terminalens utseende
...
HanuBees støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/HanuBees/themes/*.json` eller `$XDG_CONFIG_HOME/HanuBees/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.HanuBees/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.HanuBees/themes/*.json`
