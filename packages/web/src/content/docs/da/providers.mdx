---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i HanuBees.
---

import config from "../../../../config.mjs"
export const console = config.console

HanuBees bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i HanuBees-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/HanuBees/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i HanuBees-konfigurationen.
...
HanuBees Zen er en liste over modeller leveret af HanuBees-teamet, som er blevet testet og verificeret til at fungere godt med HanuBees. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med HanuBees Zen.
:::
...
Det fungerer som alle andre udbydere i HanuBees og er helt valgfrit at bruge.

---

## HanuBees Go

HanuBees Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af HanuBees-teamet, som er testet og verificeret til at fungere godt med HanuBees.

1. Kør kommandoen `/connect` i TUI, vælg `HanuBees Go`, og gå til [hanubees.com/auth](https://hanubees.com/zen).
...
Sådan bruger du Amazon Bedrock med HanuBees:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører HanuBees:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `HanuBees.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i HanuBees understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere HanuBees til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at HanuBees skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem HanuBees-konfigurationen.
...
gemme tokenet i HanuBees auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
HanuBees bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse HanuBees
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`HanuBees.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [HanuBees-gitlab-auth](https://www.npmjs.com/package/HanuBees-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `HanuBees.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med HanuBees:
...
Sådan bruger du Google Vertex AI med HanuBees:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører HanuBees.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem HanuBees, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [HanuBees-helicone-session](https://github.com/H2Shami/HanuBees-helicone-session) plugin for automatisk at logge hver HanuBees-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver HanuBees-samtale opført som en separat session.
...
Du kan konfigurere HanuBees til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere HanuBees til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere HanuBees til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for HanuBees. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/HanuBees) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med HanuBees:
...
3. Kopier API-nøglen til brug i HanuBees.
...
6. **Vigtigt**: Før du bruger skymodeller i HanuBees, skal du hente modelinformationen lokalt:
...
HanuBees Zen er en liste over testede og verificerede modeller leveret af HanuBees-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>HanuBees Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **HanuBees Zen**.

   ```txt
   /connect
   ```

3. Indtast HanuBees API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem HanuBees-konfigurationen.
...
5. Du kan også tilpasse dem gennem HanuBees-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med HanuBees:
...
Du kan også tilpasse modeller gennem HanuBees-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med HanuBees. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `HanuBees.json`-filen i projektkataloget:
...
`limit`-felterne lader HanuBees forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `HanuBees auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek HanuBees-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i HanuBees-konfigurationen.
