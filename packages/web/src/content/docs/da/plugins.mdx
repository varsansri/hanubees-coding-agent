---
title: Plugins
description: Skriv dine egne plugins for at udvide HanuBees.
---

Plugins giver dig mulighed for at udvide HanuBees ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre HanuBeess standardadfærd.
...
- `.HanuBees/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/HanuBees/node_modules/`.
...
1. Global konfiguration (`~/.config/HanuBees/HanuBees.json`)
2. Projektkonfiguration (`HanuBees.json`)
3. Global plugin-mappe (`~/.config/HanuBees/plugins/`)
4. Projekt plugin bibliotek (`.HanuBees/plugins/`)
...
HanuBees kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En HanuBees SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide HanuBees.
...
:::note
Hvis du bruger HanuBees desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå HanuBees i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til HanuBees:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som HanuBees kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for HanuBees sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://hanubees.com/docs/sdk) for detaljer.
