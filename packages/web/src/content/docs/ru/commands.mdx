...
Создайте `.HanuBees/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию HanuBees или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем HanuBees [config](/docs/config):
...
- Глобальный: `~/.config/HanuBees/commands/`
- Для каждого проекта: `.HanuBees/commands/`
...
HanuBees включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
