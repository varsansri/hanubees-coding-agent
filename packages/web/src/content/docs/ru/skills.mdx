...
Навыки агента позволяют HanuBees обнаруживать многократно используемые инструкции из вашего репозитория или домашнего каталога.
...
HanuBees выполняет поиск в следующих местах:

- Конфигурация проекта: `.HanuBees/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/HanuBees/skills/<name>/SKILL.md`.
...
Для локальных путей проекта HanuBees переходит из вашего текущего рабочего каталога, пока не достигнет рабочего дерева git.
Он загружает все соответствующие `skills/*/SKILL.md` в `.HanuBees/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/HanuBees/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.HanuBees/skills/git-release/SKILL.md` следующим образом:
...
HanuBees перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `HanuBees.json`:
...
**Для встроенных агентов** (в формате `HanuBees.json`):
