---
title: Tjener
description: Samhandle med HanuBees-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `HanuBees serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en HanuBees-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `HanuBees.local` |
...
Sett `HanuBees_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `HanuBees` som standard, eller sett `HanuBees_SERVER_USERNAME` for å overstyre det. Dette gjelder både `HanuBees serve` og `HanuBees web`.
...
Når du kjører `HanuBees` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk HanuBees-serveren til å samhandle med HanuBees programmatisk.
:::

Denne arkitekturen lar HanuBees støtte flere klienter og lar deg samhandle med HanuBees programmatisk.

Du kan kjøre `HanuBees serve` for å starte en frittstående server. Hvis du har
HanuBees TUI kjører, vil `HanuBees serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av HanuBees [IDE](/docs/ide) plugins.
...
HanuBees-serveren viser følgende APIer.
