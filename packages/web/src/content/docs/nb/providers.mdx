---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i HanuBees.
---

import config from "../../../../config.mjs"
export const console = config.console

HanuBees bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i HanuBees-konfigurasjonen.
...
i `~/.local/share/HanuBees/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i HanuBees
konfig.
...
HanuBees Zen er en liste over modeller levert av HanuBees-teamet som har vært
testet og verifisert for å fungere godt med HanuBees. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med HanuBees Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `HanuBees Zen` og gå til [hanubees.com/auth](https://hanubees.com/zen).
...
Det fungerer som alle andre leverandører i HanuBees og er helt valgfritt å bruke.

---

## HanuBees Go

HanuBees Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av HanuBees-teamet som har vært
testet og verifisert for å fungere godt med HanuBees.

1. Kjør kommandoen `/connect` i TUI, velg `HanuBees Go`, og gå til [hanubees.com/auth](https://hanubees.com/zen).
...
Slik bruker du Amazon Bedrock med HanuBees:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører HanuBees:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `HanuBees.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i HanuBees støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere HanuBees til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at HanuBees skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom HanuBees-konfigurasjonen.
...
for å lagre token i HanuBees auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
HanuBees bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse HanuBees
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`HanuBees.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [HanuBees-gitlab-auth](https://www.npmjs.com/package/HanuBees-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `HanuBees.json`:
...
Slik bruker du GitHub Copilot-abonnementet med HanuBees:
...
Slik bruker du Google Vertex AI med HanuBees:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører HanuBees.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom HanuBees, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [HanuBees-helicone-session](https://github.com/H2Shami/HanuBees-helicone-session) plugin for automatisk å logge hver HanuBees-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver HanuBees-samtale oppført som en separat økt.
...
Du kan konfigurere HanuBees for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere HanuBees for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere HanuBees for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for HanuBees. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/HanuBees) for detaljer.
:::
...
Slik bruker du Ollama Cloud med HanuBees:
...
3. Kopier API-nøkkelen for bruk i HanuBees.
...
6. **Viktig**: Før du bruker skymodeller i HanuBees, må du hente modellinformasjonen lokalt:
...
HanuBees Zen er en liste over testede og verifiserte modeller levert av HanuBees-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>HanuBees Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **HanuBees Zen**.

   ```txt
   /connect
   ```

3. Skriv inn HanuBees API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom HanuBees-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom HanuBees-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med HanuBees:
...
Du kan også tilpasse modeller gjennom HanuBees-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med HanuBees. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `HanuBees.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar HanuBees forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `HanuBees auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk HanuBees-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i HanuBees-konfigurasjonen.
