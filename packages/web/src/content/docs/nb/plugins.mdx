...
description: Skriv dine egne programtillegg for å utvide HanuBees.
---

Plugins lar deg utvide HanuBees ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til HanuBees.
...
- `.HanuBees/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/HanuBees/node_modules/`.
...
1. Global konfigurasjon (`~/.config/HanuBees/HanuBees.json`)
2. Prosjektkonfigurasjon (`HanuBees.json`)
3. Global plugin-katalog (`~/.config/HanuBees/plugins/`)
4. Prosjektpluginkatalog (`.HanuBees/plugins/`)
...
HanuBees kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En HanuBees SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide HanuBees.
...
:::note
Hvis du bruker HanuBees-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre HanuBees fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til HanuBees:
...
`tool`-hjelperen lager et tilpasset verktøy som HanuBees kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for HanuBees sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://hanubees.com/docs/sdk) for detaljer.
