...
Det er to typer agenter i HanuBees; primære agenter og underagenter.
...
HanuBees kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
HanuBees kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

HanuBees kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `HanuBees.json`:
...
Hvis ingen temperatur er spesifisert, bruker HanuBees modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale HanuBees-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i HanuBees-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [HanuBees Zen](/docs/zen), vil du bruke `HanuBees/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `HanuBees models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/varsansri/hanubees-coding-agent).
:::
