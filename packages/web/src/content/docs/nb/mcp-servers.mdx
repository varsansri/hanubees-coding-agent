...
Du kan legge til eksterne verktøy i HanuBees ved å bruke _Model Context Protocol_ eller MCP. HanuBees støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [HanuBees Config](https://hanubees.com/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="HanuBees.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/HanuBees`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
HanuBees håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil HanuBees:
...
Hvis serveren krever autentisering, vil HanuBees be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `HanuBees mcp auth <server-name>`.
...
```bash
HanuBees mcp auth my-oauth-server
```
...
```bash
HanuBees mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil HanuBees lagre tokens sikkert i `~/.local/share/HanuBees/mcp-auth.json`.
...
HanuBees mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i HanuBees, sammen med innebygde verktøy. Så du kan administrere dem gjennom HanuBees-konfigurasjonen som alle andre verktøy.
...
```json title="HanuBees.json" {11, 14-18}
{
  "$schema": "https://hanubees.com/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
HanuBees mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble HanuBees til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver HanuBees-samtale oppført som en separat økt.
