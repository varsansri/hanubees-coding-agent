...
يمكنك إضافة أدوات خارجية إلى HanuBees باستخدام _Model Context Protocol_، أو MCP. يدعم HanuBees خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات HanuBees](https://hanubees.com/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/HanuBees` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى HanuBees تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم HanuBees بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك HanuBees المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `HanuBees mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن HanuBees الرموز بشكل آمن في `~/.local/share/HanuBees/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل HanuBees إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات HanuBees مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط HanuBees بحساب Sentry الخاص بك.
