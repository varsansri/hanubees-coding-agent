...
أنشئ الملف `.HanuBees/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات HanuBees أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ HanuBees:
...
- على مستوى النظام: `~/.config/HanuBees/commands/`
- لكل مشروع: `.HanuBees/commands/`
...
يتضمن HanuBees عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
