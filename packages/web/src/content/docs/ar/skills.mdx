...
تتيح مهارات الوكيل لـ HanuBees اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث HanuBees في هذه المواقع:

- إعدادات المشروع: `.HanuBees/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/HanuBees/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج HanuBees صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.HanuBees/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/HanuBees/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.HanuBees/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض HanuBees المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `HanuBees.json`:
...
**للوكلاء المدمجين** (في `HanuBees.json`):
