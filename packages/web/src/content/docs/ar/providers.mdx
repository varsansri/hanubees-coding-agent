---
title: المزوّدون
description: استخدام أي موفّر LLM في HanuBees.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم HanuBees ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات HanuBees.
...
في `~/.local/share/HanuBees/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات HanuBees.
...
HanuBees Zen هي قائمة نماذج يوفّرها فريق HanuBees وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع HanuBees. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع HanuBees Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر HanuBees، ثم انتقل إلى [hanubees.com/auth](https://hanubees.com/auth).
...
يعمل مثل أي مزوّد آخر في HanuBees واستخدامه اختياري بالكامل.

---

## HanuBees Go

HanuBees Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق HanuBees، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع HanuBees.

1. شغّل الأمر `/connect` في TUI، واختر `HanuBees Go`، ثم انتقل إلى [hanubees.com/auth](https://hanubees.com/zen).
...
لاستخدام Amazon Bedrock مع HanuBees:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل HanuBees:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `HanuBees.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل HanuBees غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين HanuBees لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل HanuBees بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات HanuBees.
...
تخزين الرمز في مساحة تخزين مصادقة HanuBees.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم HanuBees نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر HanuBees
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`HanuBees.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [HanuBees-gitlab-auth](https://www.npmjs.com/package/HanuBees-gitlab-auth).

##### التهيئة

خصّص عبر `HanuBees.json`:
...
لاستخدام اشتراك GitHub Copilot مع HanuBees:
...
لاستخدام Google Vertex AI مع HanuBees:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر HanuBees، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [HanuBees-helicone-session](https://github.com/H2Shami/HanuBees-helicone-session) لتسجيل كل محادثة HanuBees تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة HanuBees مدرجة كجلسة مستقلة.
...
يمكنك ضبط HanuBees لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط HanuBees لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط HanuBees لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع HanuBees. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/HanuBees) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع HanuBees:
...
3. انسخ مفتاح API لاستخدامه في HanuBees.
...
6. **مهم**: قبل استخدام نماذج السحابة في HanuBees، يجب سحب معلومات النموذج محليا:
...
HanuBees Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق HanuBees. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>HanuBees Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **HanuBees Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ HanuBees.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات HanuBees.
...
5. يمكنك أيضا تخصيصها عبر إعدادات HanuBees. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع HanuBees:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات HanuBees. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع HanuBees. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `HanuBees.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ HanuBees بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `HanuBees auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات HanuBees وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات HanuBees.
