...
Dzięki HanuBees możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie HanuBees używa naszego własnego motywu `HanuBees`.
...
HanuBees ma kilka wbudowanych motywów.
...
- Chcą, aby HanuBees pasował do wyglądu ich terminala
...
HanuBees obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/HanuBees/themes/*.json` lub `$XDG_CONFIG_HOME/HanuBees/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.HanuBees/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.HanuBees/themes/*.json`
