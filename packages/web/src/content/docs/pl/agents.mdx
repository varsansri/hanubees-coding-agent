...
W HanuBees obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
HanuBees zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
HanuBees ma trzech wbudowanych subagentów: **General**, **Explore** i **Scout**. Przyjrzymy się im poniżej.

---

## Wbudowane

HanuBees ma dwa wbudowane agenty główne i trzech wbudowanych subagentów.
...
Agent tylko do odczytu do pracy z zewnętrzną dokumentacją i badaniem zależności. Używaj go, gdy chcesz sklonować repozytorium zależności do zarządzanej pamięci podręcznej HanuBees, przejrzeć kod źródłowy biblioteki albo porównać lokalny kod z implementacjami upstream bez modyfikowania swojego workspace.
...
Skonfiguruj agenta w pliku `HanuBees.json`:
...
- Globalnie: `~/.config/HanuBees/agents/`
- W projekcie: `.HanuBees/agents/`
...
Jeśli nie zostanie skonfigurowana, HanuBees używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji HanuBees, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji HanuBees w `provider/model-id`. Na przykład, [HanuBees Zen](/docs/zen), użyj `HanuBees/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `HanuBees models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/varsansri/hanubees-coding-agent).
:::
