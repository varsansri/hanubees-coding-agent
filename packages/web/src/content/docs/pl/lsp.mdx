---
title: Serwery LSP
description: HanuBees integruje się z serwerami LSP.
---

HanuBees może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

HanuBees posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `HanuBees_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i HanuBees otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji HanuBees.
