...
Umiejętności agenta pozwalają HanuBees odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
HanuBees przeszukuje te lokalizacje:

- Project config: `.HanuBees/skills/<name>/SKILL.md`
- Global config: `~/.config/HanuBees/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu HanuBees przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.HanuBees/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/HanuBees/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.HanuBees/skills/git-release/SKILL.md` w ten sposób:
...
HanuBees wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `HanuBees.json`:
...
**Dla agentów wbudowanych** (w `HanuBees.json`):
