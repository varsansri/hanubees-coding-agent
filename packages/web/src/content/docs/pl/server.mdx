---
title: Serwer
description: Interakcja z serwerem HanuBees za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `HanuBees serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient HanuBees.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `HanuBees.local` |
...
Ustaw `HanuBees_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `HanuBees` lub ustaw `HanuBees_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `HanuBees serve`, jak i `HanuBees web`.
...
Kiedy uruchomisz `HanuBees`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera HanuBees do programowej interakcji z HanuBees.
:::

Ta architektura umożliwia obsługę wielu klientów przez HanuBees i programową interakcję z kodem otwartym.

Możesz uruchomić `HanuBees serve`, aby uruchomić samodzielny serwer. Jeśli masz
HanuBees TUI działa, `HanuBees serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki HanuBees [IDE](/docs/ide).
...
Serwer HanuBees udostępnia następujące interfejsy API.
