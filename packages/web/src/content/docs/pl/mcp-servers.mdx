...
Możesz dodać dodatkowe informacje do HanuBees za pomocą _Model Context Protocol_, w skrócie MCP. HanuBees obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [HanuBees Config](https://hanubees.com/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/HanuBees`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
HanuBees automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, HanuBees:
...
Jeśli serwer wymaga uwierzytelnienia, HanuBees poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `HanuBees mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji HanuBees bezpieczne przechowa tokeny w `~/.local/share/HanuBees/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w HanuBees, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie HanuBees, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć HanuBees z kontem Sentry.
