---
title: Konfiguracja
description: Ustawienia HanuBees w pliku JSON.
---

Możesz dostosować HanuBees za pomocą pliku konfiguracyjnego JSON.

---

## Format

HanuBees obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "HanuBees"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/HanuBees`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/HanuBees/HanuBees.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `HanuBees_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`HanuBees.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.HanuBees`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `HanuBees_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.HanuBees` i `~/.config/HanuBees` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/HanuBees`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację HanuBees w `~/.config/HanuBees/HanuBees.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/HanuBees/tui.json`.
...
Dodaj `HanuBees.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy HanuBees się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `HanuBees_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `HanuBees_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.HanuBees` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.HanuBees`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`hanubees.com/config.json`**](https://hanubees.com/config.json).
...
Użyj `HanuBees_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `HanuBees.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `HanuBees serve` i `HanuBees web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer HanuBees.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `HanuBees.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie HanuBees wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/HanuBees/agents/` lub `.HanuBees/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, HanuBees powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`HanuBees run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/HanuBees/commands/` lub `.HanuBees/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
HanuBees może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy HanuBees nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie HanuBees **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają HanuBees o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.HanuBees/plugins/` lub `~/.config/HanuBees/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie HanuBees tylko do określonych dostawców.
