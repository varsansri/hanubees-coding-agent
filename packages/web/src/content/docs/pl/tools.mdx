...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. HanuBees posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w HanuBees.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `HanuBees_EXPERIMENTAL_LSP_TOOL=true` (lub `HanuBees_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy HanuBees lub gdy zmienna środowiskowa `HanuBees_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu HanuBees:
