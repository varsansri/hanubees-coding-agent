---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w HanuBees.
---

import config from "../../../../config.mjs"
export const console = config.console

HanuBees używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji HanuBees.
...
w `~/.local/share/HanuBees/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim HanuBees
config.
...
HanuBees Zen to lista modeli dostarczonych przez zespół HanuBees, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z HanuBees. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od HanuBees Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `HanuBees Zen` i przejdź do [hanubees.com/auth](https://hanubees.com/zen).
...
Działa jak każdy inny dostawca w HanuBees i jest całkowicie opcjonalny w użyciu.

---

## HanuBees Go

HanuBees Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół HanuBees, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z HanuBees.

1. Uruchom polecenie `/connect` w TUI, wybierz `HanuBees Go` i przejdź do [hanubees.com/auth](https://hanubees.com/zen).
...
Aby używać Amazon Bedrock z HanuBees:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania HanuBees:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `HanuBees.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w HanuBees nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować HanuBees do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod HanuBees działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji HanuBees.
...
HanuBees używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować HanuBees
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`HanuBees.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [HanuBees-gitlab-auth](https://www.npmjs.com/package/HanuBees-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem HanuBees:
...
Aby używać Google Vertex AI z HanuBees:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania HanuBees.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą HanuBees, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [HanuBees-helicone-session](https://github.com/H2Shami/HanuBees-helicone-session), aby automatycznie rejestrować każdą konwersację HanuBees jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja HanuBees będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować HanuBees tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować HanuBees tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować HanuBees tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem HanuBees. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/HanuBees).
:::
...
Aby korzystać z Ollama Cloud z HanuBees:
...
3. Skopiuj klucz API do użycia w HanuBees.
...
6. **Ważne**: Przed użyciem modeli chmurowych w HanuBees musisz pobrać informacje o modelu lokalnie:
...
HanuBees Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół HanuBees. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>HanuBees Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **HanuBees Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API HanuBees.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji HanuBees.
...
5. Możesz je także dostosować za pomocą konfiguracji HanuBees. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z HanuBees:
...
Możesz także dostosować modele za pomocą konfiguracji HanuBees. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z HanuBees. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `HanuBees.json` w katalogu projektu:
...
Pola `limit` pozwalają HanuBees zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `HanuBees auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację HanuBees i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji HanuBees.
