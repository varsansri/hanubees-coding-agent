...
Da biste otklonili probleme s HanuBees, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\HanuBees\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `HanuBees --log-level DEBUG`.

---

## Pohrana

HanuBees pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/HanuBees/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\HanuBees`
...
HanuBees Desktop pokreće lokalni HanuBees server (`HanuBees-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `HanuBees` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/HanuBees/HanuBees.jsonc` (ili `~/.config/HanuBees/HanuBees.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/HanuBees/HanuBees.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\HanuBees\HanuBees.jsonc`
...
HanuBees također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/HanuBees/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\HanuBees\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga HanuBees mogao ponovo izgraditi.

1. Potpuno zatvorite HanuBees Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/HanuBees`
- **Linux**: obrišite `~/.cache/HanuBees` (ili pokrenite `rm -rf ~/.cache/HanuBees`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\HanuBees`

3. Ponovo pokrenite HanuBees Desktop.

---

### Rješavanje problema sa vezom na serveru

HanuBees Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `HanuBees.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `HanuBees_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `HanuBees_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, HanuBees Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa HanuBees karakteristikama.

---

### Obavještenja se ne prikazuju

HanuBees Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za HanuBees u postavkama vašeg OS-a, i
...
1. Zatvorite HanuBees Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka HanuBees Desktop aplikacije):

- `HanuBees.settings.dat` (URL zadanog servera za desktop)
- `HanuBees.global.dat` i `HanuBees.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `HanuBees models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\HanuBees`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. HanuBees dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\HanuBees`

2. Ponovo pokrenite HanuBees da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti HanuBees da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
HanuBees će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
