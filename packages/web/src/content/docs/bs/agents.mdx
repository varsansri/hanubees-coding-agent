...
Postoje dvije vrste agenata u HanuBees: primarni agenti i podagenti.
...
HanuBees dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
HanuBees dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

HanuBees dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u HanuBees-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `HanuBees.json`:
...
- Globalno: `~/.config/HanuBees/agents/`
- Po projektu: `.HanuBees/agents/`
...
Ako temperatura nije navedena, HanuBees koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu HanuBees konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj HanuBees konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [HanuBees Zen](/docs/zen), koristili biste `HanuBees/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `HanuBees models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/varsansri/hanubees-coding-agent).
:::
