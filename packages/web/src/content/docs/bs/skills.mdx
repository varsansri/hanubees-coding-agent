...
Agent skills omogucavaju HanuBees da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
HanuBees pretrazuje ove lokacije:

- Konfiguracija projekta: `.HanuBees/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/HanuBees/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, HanuBees ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.HanuBees/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/HanuBees/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.HanuBees/skills/git-release/SKILL.md` ovako:
...
HanuBees navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `HanuBees.json`:
...
**Za ugradene agente** (u `HanuBees.json`):
