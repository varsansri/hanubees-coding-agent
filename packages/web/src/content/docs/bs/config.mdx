---
title: Konfiguracija
description: Korištenje HanuBees JSON konfiguracije.
---

Možete konfigurirati HanuBees koristeći JSON konfiguracijski fajl.

---

## Format

HanuBees podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/HanuBees`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/HanuBees/HanuBees.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`HanuBees_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`HanuBees.json` u projektu) - postavke specifične za projekat
5. **`.HanuBees` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`HanuBees_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.HanuBees` i `~/.config/HanuBees` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/HanuBees` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu HanuBees konfiguraciju u `~/.config/HanuBees/HanuBees.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/HanuBees/tui.json`.
...
Dodajte `HanuBees.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se HanuBees pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `HanuBees_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `HanuBees_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.HanuBees` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.HanuBees`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`hanubees.com/config.json`**](https://hanubees.com/config.json).

TUI konfiguracija koristi [**`hanubees.com/tui.json`**](https://hanubees.com/tui.json).
...
Koristite `HanuBees_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `HanuBees.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `HanuBees serve` i `HanuBees web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš HanuBees server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `HanuBees.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj HanuBees konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, HanuBees pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/HanuBees/agents/` ili `.HanuBees/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, HanuBees će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`HanuBees run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/HanuBees/commands/` ili `.HanuBees/commands/`. [Saznajte više ovdje](/docs/commands).
...
HanuBees će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, HanuBees **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju HanuBees sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.HanuBees/plugins/` ili `~/.config/HanuBees/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite HanuBees da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
