---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u HanuBees.
---

import config from "../../../../config.mjs"
export const console = config.console

HanuBees koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj HanuBees konfiguraciji.
...
u `~/.local/share/HanuBees/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem HanuBees
config.
...
HanuBees Zen je lista modela koje je obezbedio HanuBees tim koji su bili
testirano i potvrđeno da dobro radi sa HanuBees. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa HanuBees Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite HanuBees i idite na [hanubees.com/auth](https://hanubees.com/auth).
...
Radi kao i svaki drugi provajder u HanuBees i potpuno je opcionalan za korištenje.

---

## HanuBees Go

HanuBees Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža HanuBees tim i koji su testirani i verificirani da dobro rade s HanuBees-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `HanuBees Go` i idite na [hanubees.com/auth](https://hanubees.com/zen).
...
Da biste koristili Amazon Bedrock s HanuBees:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete HanuBees:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `HanuBees.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u HanuBees.
:::
...
Možete konfigurirati HanuBees za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi HanuBees ispravno radio.
   :::
...
Također možete dodati modele kroz svoju HanuBees konfiguraciju.
...
da pohrani token u HanuBees auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
HanuBees koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate HanuBees
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`HanuBees.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [HanuBees-gitlab-auth](https://www.npmjs.com/package/HanuBees-gitlab-auth).

##### Konfiguracija

Prilagodite putem `HanuBees.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s HanuBees:
...
Za korištenje Google Vertex AI s HanuBees:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće HanuBees.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem HanuBeesa, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [HanuBees-helicone-session](https://github.com/H2Shami/HanuBees-helicone-session) da automatski prijavite svaki HanuBees razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki HanuBees razgovor naveden kao zasebna sesija.
...
Možete konfigurirati HanuBees za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati HanuBees za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati HanuBees za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za HanuBees. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/HanuBees) za detalje.
:::
...
Da biste koristili Ollama Cloud s HanuBees:
...
3. Kopirajte API ključ za korištenje u HanuBees.
...
6. **Važno**: Prije upotrebe modela oblaka u HanuBees, morate lokalno povući informacije o modelu:
...
HanuBees Zen je lista testiranih i verifikovanih modela koju je obezbedio HanuBees tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>HanuBees Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **HanuBees Zen**.

```txt
   /connect
```

3. Unesite svoj HanuBees API ključ.
...
Također možete dodati dodatne modele putem vaše HanuBees konfiguracije.
...
5. Također ih možete prilagoditi putem vaše HanuBees konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa HanuBeesom:
...
Također možete prilagoditi modele kroz svoju HanuBees konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s HanuBees-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `HanuBees.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju HanuBees da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `HanuBees auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite HanuBees konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj HanuBees konfiguraciji.
