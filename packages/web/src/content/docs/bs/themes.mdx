...
U HanuBees mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, HanuBees koristi nasu `HanuBees` temu.
...
HanuBees dolazi sa vise ugradenih tema.
...
- Zele da HanuBees odgovara izgledu njihovog terminala
...
HanuBees podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/HanuBees/themes/*.json` ili `$XDG_CONFIG_HOME/HanuBees/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.HanuBees/themes/*.json`
4. **Trenutni radni direktorij** - `./.HanuBees/themes/*.json`
