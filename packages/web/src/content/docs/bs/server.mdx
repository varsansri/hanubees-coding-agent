---
title: Server
description: Komunicirajte s HanuBees serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `HanuBees serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji HanuBees klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `HanuBees.local` |
...
Postavite `HanuBees_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `HanuBees`, ili postavite `HanuBees_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `HanuBees serve` i za `HanuBees web`.
...
Kada pokrenete `HanuBees`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite HanuBees server za programsku interakciju sa HanuBees-om.
:::

Ova arhitektura omogucava HanuBees podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `HanuBees serve` da startate standalone server. Ako je HanuBees TUI vec pokrenut, `HanuBees serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste HanuBees [IDE](/docs/ide) pluginovi.
...
HanuBees server izlaže sljedece API-je.
