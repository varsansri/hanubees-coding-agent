---
title: Dodaci
description: Napišite vlastite dodatke za proširenje HanuBees.
---

Dodaci vam omogućavaju da proširite HanuBees spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja HanuBees.
...
- `.HanuBees/plugins/` - Dodaci na nivou projekta
- `~/.config/HanuBees/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/HanuBees/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/HanuBees/HanuBees.json`)
2. Konfiguracija projekta (`HanuBees.json`)
3. Globalni direktorij dodataka (`~/.config/HanuBees/plugins/`)
4. Direktorij dodataka projekta (`.HanuBees/plugins/`)
...
HanuBees pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: HanuBees SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje HanuBees.
...
Spriječite HanuBees da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u HanuBees:
...
Pomoćnik `tool` kreira prilagođeni alat koji HanuBees može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za HanuBees zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://hanubees.com/docs/sdk) za detalje.
