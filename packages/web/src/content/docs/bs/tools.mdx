...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. HanuBees dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u HanuBees.
...
:::note
Ovaj alat je dostupan samo kada je `HanuBees_EXPERIMENTAL_LSP_TOOL=true` (ili `HanuBees_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz HanuBees provajdera ili kada je varijabla `HanuBees_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju HanuBees:
