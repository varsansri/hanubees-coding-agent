...
Možete dodati vanjske alate u HanuBees koristeći _Model Context Protocol_, ili MCP. HanuBees podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [HanuBees Config](https://hanubees.com/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/HanuBees`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
HanuBees automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, HanuBees će:
...
Ako server zahtijeva autentifikaciju, HanuBees će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `HanuBees mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, HanuBees sigurno čuva tokene u `~/.local/share/HanuBees/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u HanuBees, zajedno s ugrađenim alatima. Možete njima upravljati kroz HanuBees konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete HanuBees sa vašim Sentry nalogom.
