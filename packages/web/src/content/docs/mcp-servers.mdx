...
You can add external tools to HanuBees using the _Model Context Protocol_, or MCP. HanuBees supports both local and remote servers.
...
You can define MCP servers in your [HanuBees Config](https://hanubees.com/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/HanuBees` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
HanuBees automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, HanuBees will:
...
If the server requires authentication, HanuBees will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `HanuBees mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, HanuBees will store the tokens securely in `~/.local/share/HanuBees/mcp-auth.json`.
...
Your MCPs are available as tools in HanuBees, alongside built-in tools. So you can manage them through the HanuBees config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect HanuBees to your Sentry account.
