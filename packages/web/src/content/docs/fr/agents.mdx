...
Il existe deux types d'agents dans HanuBees ; agents primaires et sous-agents.
...
HanuBees est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
HanuBees est livré avec trois sous-agents intégrés, **General**, **Explore** et **Scout**. Nous les verrons ci-dessous.

---

## Agents intégrés

HanuBees est livré avec deux agents primaires intégrés et trois sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `HanuBees.json` :
...
Si aucune température n'est spécifiée, HanuBees utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale HanuBees et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration HanuBees utilise le format `provider/model-id`. Par exemple, si vous utilisez [HanuBees Zen](/docs/zen), vous utiliserez `HanuBees/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `HanuBees models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/varsansri/hanubees-coding-agent).
:::
