---
title: Plugins
description: Écrivez vos propres plugins pour étendre HanuBees.
---

Les plugins vous permettent d'étendre HanuBees en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de HanuBees.
...
- `.HanuBees/plugins/` - Plugins au niveau du projet
- `~/.config/HanuBees/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/HanuBees/node_modules/`.
...
1. Configuration globale (`~/.config/HanuBees/HanuBees.json`)
2. Configuration du projet (`HanuBees.json`)
3. Répertoire global des plugins (`~/.config/HanuBees/plugins/`)
4. Répertoire des plugins du projet (`.HanuBees/plugins/`)
...
HanuBees exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK HanuBees pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre HanuBees.
...
:::note
Si vous utilisez l'application de bureau HanuBees, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez HanuBees de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à HanuBees :
...
L'assistant `tool` crée un outil personnalisé que HanuBees peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour HanuBees aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://hanubees.com/docs/sdk) pour plus de détails.
