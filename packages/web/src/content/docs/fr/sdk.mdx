---
title: SDK
description: Client JS de type sécurisé pour le serveur HanuBees.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK HanuBees JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler HanuBees par programme.
...
Créez une instance de HanuBees :

```javascript
import { createHanuBees } from "@hanubees/sdk"

const { client } = await createHanuBees()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `HanuBees.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :

```javascript
import { createHanuBees } from "@hanubees/sdk"

const HanuBees = await createHanuBees({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${HanuBees.server.url}`)

HanuBees.server.close()
```

## Client uniquement

Si vous disposez déjà d'une instance en cours d'exécution de HanuBees, vous pouvez créer une instance client pour vous y connecter :

```javascript
import { createHanuBeesClient } from "@hanubees/sdk"

const client = createHanuBeesClient({
...
```typescript
import type { Session, Message, Part } from "@hanubees/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*HanuBees" },
})
