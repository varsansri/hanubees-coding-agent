...
Avec HanuBees, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, HanuBees utilise notre propre thème `HanuBees`.
...
HanuBees est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que HanuBees corresponde à l'apparence de leur terminal
...
HanuBees prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/HanuBees/themes/*.json` ou `$XDG_CONFIG_HOME/HanuBees/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.HanuBees/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.HanuBees/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/HanuBees/themes
vim ~/.config/HanuBees/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .HanuBees/themes
vim .HanuBees/themes/my-theme.json
```
...
{
  "$schema": "https://hanubees.com/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
