---
title: Serveur
description: Interagissez avec le serveur HanuBees via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `HanuBees serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client HanuBees peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `HanuBees.local` |
...
Définissez `HanuBees_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `HanuBees`, ou définissez `HanuBees_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `HanuBees serve` et à `HanuBees web`.
...
Lorsque vous exécutez `HanuBees`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur HanuBees pour interagir avec HanuBees par programmation.
:::

Cette architecture permet à HanuBees de prendre en charge plusieurs clients et vous permet d'interagir avec HanuBees par programme.

Vous pouvez exécuter `HanuBees serve` pour démarrer un serveur autonome. Si vous avez le
HanuBees TUI en cours d'exécution, `HanuBees serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins HanuBees [IDE](/docs/ide).
...
Le serveur HanuBees expose les API suivantes.
