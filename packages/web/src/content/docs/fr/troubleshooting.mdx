...
Pour déboguer les problèmes avec HanuBees, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\HanuBees\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `HanuBees --log-level DEBUG`.

---

## Stockage

HanuBees stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/HanuBees/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\HanuBees`
...
HanuBees Desktop exécute un serveur HanuBees local (le side-car `HanuBees-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `HanuBees` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/HanuBees/HanuBees.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\HanuBees\HanuBees.jsonc`
...
HanuBees peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/HanuBees/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\HanuBees\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que HanuBees puisse le reconstruire.

1. Quittez complètement HanuBees Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/HanuBees`
- **Linux** : supprimez `~/.cache/HanuBees` (ou exécutez `rm -rf ~/.cache/HanuBees`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\HanuBees`

3. Redémarrez le bureau HanuBees.

---

### Résoudre les problèmes de connexion au serveur

HanuBees Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `HanuBees.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `HanuBees_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `HanuBees_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, HanuBees Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de HanuBees.

---

### Les notifications ne s'affichent pas

HanuBees Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour HanuBees dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau HanuBees.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application HanuBees Desktop) :

- `HanuBees.settings.dat` (serveur de bureau par défaut URL)
- `HanuBees.global.dat` et `HanuBees.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec HanuBees :
...
3. Assurez-vous d'avoir la dernière version avec `HanuBees upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `HanuBees models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\HanuBees`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. HanuBees installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\HanuBees`

2. Redémarrez HanuBees pour réinstaller les derniers packages du fournisseur

Cela forcera HanuBees à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
HanuBees détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
