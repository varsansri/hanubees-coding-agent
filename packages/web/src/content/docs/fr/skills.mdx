...
Les compétences d'agent permettent à HanuBees de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
HanuBees recherche ces emplacements :

- Configuration du projet : `.HanuBees/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/HanuBees/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, HanuBees remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.HanuBees/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/HanuBees/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.HanuBees/skills/git-release/SKILL.md` comme ceci :
...
HanuBees répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `HanuBees.json` :
...
**Pour les agents intégrés** (dans `HanuBees.json`) :
