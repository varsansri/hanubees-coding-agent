...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez HanuBees.
...
Voici plusieurs modèles qui fonctionnent bien avec HanuBees, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration HanuBees.
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [HanuBees Zen](/docs/zen), vous utiliserez `HanuBees/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="HanuBees.jsonc" {7-12,19-24}
{
  "$schema": "https://hanubees.com/config.json",
  "provider": {
    "openai": {
...
```jsonc title="HanuBees.jsonc" {6-21}
{
  "$schema": "https://hanubees.com/config.json",
  "provider": {
    "HanuBees": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. HanuBees est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

HanuBees est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="HanuBees.jsonc" {7-18}
{
  "$schema": "https://hanubees.com/config.json",
  "provider": {
    "openai": {
...
Lorsque HanuBees démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration HanuBees.
