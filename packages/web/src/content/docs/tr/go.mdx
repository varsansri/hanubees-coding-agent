...
HanuBees Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, HanuBees'daki diğer sağlayıcılar gibi çalışır. HanuBees Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve HanuBees'u kullanmak için bunu kullanmanıza gerek yoktur.
...
HanuBees Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

HanuBees Go, HanuBees'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>HanuBees Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `HanuBees Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye HanuBees Go'ya abone olabilir.
:::
...
HanuBees Go aşağıdaki limitleri içerir:
...
HanuBees yapılandırmanızdaki [model id](/docs/config/#models) formatı `HanuBees-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `HanuBees-go/kimi-k2.7-code` kullanmalısınız.
...
HanuBees Go'yu şu amaçlarla oluşturduk:
...
4. HanuBees ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
