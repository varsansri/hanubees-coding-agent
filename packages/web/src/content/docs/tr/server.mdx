---
title: Sunucu
description: HanuBees sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`HanuBees serve` komutu, HanuBees istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `HanuBees.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `HanuBees_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `HanuBees` değeridir; değiştirmek isterseniz `HanuBees_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `HanuBees serve` hem de `HanuBees web` için geçerlidir.
...
`HanuBees` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
HanuBees ile programatik etkileşim için HanuBees sunucusunu kullanın.
:::

Bu mimari, HanuBees'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`HanuBees serve` ile bağımsız bir sunucu başlatabilirsiniz. HanuBees TUI açıksa, `HanuBees serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum HanuBees [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
HanuBees sunucusu aşağıdaki API'leri sunar.
