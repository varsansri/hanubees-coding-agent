---
title: Eklentiler
description: HanuBees'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek HanuBees'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya HanuBees'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.HanuBees/plugins/` - Proje düzeyinde eklentiler
- `~/.config/HanuBees/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/HanuBees/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/HanuBees/HanuBees.json`)
2. Project config (`HanuBees.json`)
3. Global eklenti dizini (`~/.config/HanuBees/plugins/`)
4. Proje eklenti dizini (`.HanuBees/plugins/`)
...
HanuBees bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir HanuBees SDK istemcisi.
...
HanuBees'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
HanuBees masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
HanuBees'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca HanuBees'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, HanuBees'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra HanuBees'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://hanubees.com/docs/sdk) bakın.
