...
HanuBees ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak HanuBees kendi `HanuBees` temasını kullanır.
...
HanuBees birden fazla yerleşik temayla gelir.
...
- HanuBees'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
HanuBees, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/HanuBees/themes/*.json` veya `$XDG_CONFIG_HOME/HanuBees/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.HanuBees/themes/*.json`
4. **Geçerli çalışma dizini** - `./.HanuBees/themes/*.json`
