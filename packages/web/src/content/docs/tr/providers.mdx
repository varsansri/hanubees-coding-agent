---
title: Sağlayıcılar
description: HanuBees'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

HanuBees, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı HanuBees yapılandırmanızda yapılandırın.
...
`~/.local/share/HanuBees/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları HanuBees'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
HanuBees Zen, HanuBees ekibi tarafından sağlanan modellerin bir listesidir.
HanuBees ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz HanuBees Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, HanuBees'u seçin ve [hanubees.com/auth](https://hanubees.com/auth)'ye gidin.
...
HanuBees'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## HanuBees Go

HanuBees Go, HanuBees ile iyi çalıştığı test edilmiş ve doğrulanmış, HanuBees ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `HanuBees Go`'yu seçin ve [hanubees.com/auth](https://hanubees.com/zen) adresine gidin.
...
Amazon Bedrock'u HanuBees ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   HanuBees'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `HanuBees.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi HanuBees'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
HanuBees'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   HanuBees'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   HanuBees yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
HanuBees, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. HanuBees'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`HanuBees.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [HanuBees-gitlab-auth](https://www.npmjs.com/package/HanuBees-gitlab-auth) ana sayfasında.

##### Yapılandırma

`HanuBees.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi HanuBees'la kullanmak için:
...
Google Vertex AI'yi HanuBees ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları HanuBees çalıştırılırken ayarlayın.
...
Helicone'un HanuBees aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her HanuBees görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [HanuBees-helicone-session](https://github.com/H2Shami/HanuBees-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her HanuBees görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
HanuBees'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla HanuBees'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla HanuBees'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini HanuBees için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/HanuBees)'a bakın.
:::
...
Ollama Cloud'u HanuBees ile kullanmak için:
...
3. HanuBees'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: HanuBees'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
HanuBees Zen, HanuBees ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>HanuBees Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **HanuBees Zen**'i arayın.

   ```txt
   /connect
   ```

3. HanuBees API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   HanuBees yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca HanuBees yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi HanuBees ile kullanmak için:
...
Modelleri HanuBees yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı HanuBees ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `HanuBees.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları HanuBees'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `HanuBees auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için HanuBees yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, HanuBees yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
