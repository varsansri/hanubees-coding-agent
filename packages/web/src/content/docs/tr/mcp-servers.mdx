...
_Model Bağlam Protokolü_ veya MCP kullanarak HanuBees'a harici araçlar ekleyebilirsiniz. HanuBees hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [HanuBees Config](https://hanubees.com/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/HanuBees` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
HanuBees, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde HanuBees şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa HanuBees, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `HanuBees mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından HanuBees, belirteçleri `~/.local/share/HanuBees/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra HanuBees'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi HanuBees yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve HanuBees'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
