...
HanuBees'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
HanuBees, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
HanuBees, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

HanuBees iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `HanuBees.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse HanuBees modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global HanuBees yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
HanuBees yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [HanuBees Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `HanuBees/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `HanuBees models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/varsansri/hanubees-coding-agent).
:::
