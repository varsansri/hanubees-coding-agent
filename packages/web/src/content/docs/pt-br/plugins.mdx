---
title: Plugins
description: Escreva seus próprios plugins para estender o HanuBees.
---

Plugins permitem que você estenda o HanuBees conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do HanuBees.
...
- `.HanuBees/plugins/` - Plugins em nível de projeto
- `~/.config/HanuBees/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/HanuBees/node_modules/`.
...
1. Configuração global (`~/.config/HanuBees/HanuBees.json`)
2. Configuração do projeto (`HanuBees.json`)
3. Diretório de plugins global (`~/.config/HanuBees/plugins/`)
4. Diretório de plugins do projeto (`.HanuBees/plugins/`)
...
O HanuBees executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do HanuBees para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o HanuBees.
...
:::note
Se você estiver usando o aplicativo desktop HanuBees, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o HanuBees de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao HanuBees:
...
O helper `tool` cria uma ferramenta personalizada que o HanuBees pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o HanuBees junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://hanubees.com/docs/sdk) para detalhes.
