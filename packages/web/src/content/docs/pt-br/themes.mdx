...
Com o HanuBees, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o HanuBees usa nosso próprio tema `HanuBees`.
...
O HanuBees vem com vários temas embutidos.
...
- Querem que o HanuBees corresponda à aparência do seu terminal
...
O HanuBees suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/HanuBees/themes/*.json` ou `$XDG_CONFIG_HOME/HanuBees/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.HanuBees/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.HanuBees/themes/*.json`
