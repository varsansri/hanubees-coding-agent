...
As ferramentas permitem que o LLM execute ações em sua base de código. O HanuBees vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no HanuBees.
...
:::note
Esta ferramenta está disponível apenas quando `HanuBees_EXPERIMENTAL_LSP_TOOL=true` (ou `HanuBees_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor HanuBees ou quando a variável de ambiente `HanuBees_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o HanuBees:
