---
title: TUI
description: Usando a interface de usuário de terminal HanuBees.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

O HanuBees fornece uma interface de terminal interativa ou TUI para trabalhar em seus projetos com um LLM.

Executar HanuBees inicia o TUI para o diretório atual.
...
Ao usar o TUI do HanuBees, você pode digitar `/` seguido pelo nome de um comando para executar ações rapidamente. Por exemplo:
...
Adicione um provedor ao HanuBees. Permite que você selecione entre os provedores disponíveis e adicione suas chaves de API.
...
Saia do HanuBees. _Aliases_: `/quit`, `/q`
...
Isso é separado do `HanuBees.json`, que configura o comportamento do servidor/runtime.
...
- `leader_timeout` - Controla por quanto tempo o HanuBees espera depois da leader key. O padrão é `2000`.
...
Use `HanuBees_TUI_CONFIG` para carregar um caminho de configuração TUI personalizado.

### Attention

Attention permite que o TUI avise você quando o HanuBees espera uma resposta, precisa da aprovação de uma permissão, encontra um erro de sessão ou conclui uma sessão. Ative com `attention.enabled`; eventos integrados reproduzem sons quando acontecem. As notificações de desktop só são enviadas quando a janela do terminal não está em foco e não são usadas para eventos de subagent.
...
- `sound_pack` - ID do sound pack a ser usado. O padrão é `HanuBees.default`.
