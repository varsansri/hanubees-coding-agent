...
Existem dois tipos de agentes no HanuBees; agentes primários e subagentes.
...
HanuBees vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
HanuBees vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

HanuBees vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do HanuBees, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `HanuBees.json`:
...
- Global: `~/.config/HanuBees/agents/`
- Por projeto: `.HanuBees/agents/`

```markdown title="~/.config/HanuBees/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o HanuBees usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do HanuBees quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do HanuBees usa o formato `provider/model-id`. Por exemplo, se você estiver usando [HanuBees Zen](/docs/zen), você usaria `HanuBees/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="HanuBees.json" {3-6,9-12}
{
  "$schema": "https://hanubees.com/config.json",
  "tools": {
    "write": true,
...
```json title="HanuBees.json" {3-5,8-10}
{
  "$schema": "https://hanubees.com/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `HanuBees models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/varsansri/hanubees-coding-agent).
:::
...
```markdown title="~/.config/HanuBees/agents/docs-writer.md"
---
...
```markdown title="~/.config/HanuBees/agents/security-auditor.md"
---
