---
title: Servidor
description: Interaja com o servidor HanuBees via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `HanuBees serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente HanuBees pode usar.
...
```bash
HanuBees serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `HanuBees.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
HanuBees serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `HanuBees_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `HanuBees`, ou defina `HanuBees_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `HanuBees serve` quanto ao `HanuBees web`.

```bash
HanuBees_SERVER_PASSWORD=your-password HanuBees serve
```
...
Quando você executa `HanuBees`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor HanuBees para interagir com o HanuBees programaticamente.
:::

Essa arquitetura permite que o HanuBees suporte múltiplos clientes e permite que você interaja com o HanuBees programaticamente.

Você pode executar `HanuBees serve` para iniciar um servidor autônomo. Se você tiver o TUI do HanuBees em execução, `HanuBees serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do HanuBees [IDE](/docs/ide).
...
O servidor HanuBees expõe as seguintes APIs.
