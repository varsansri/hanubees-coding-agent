...
As habilidades do agente permitem que o HanuBees descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O HanuBees pesquisa nesses locais:

- Configuração do projeto: `.HanuBees/skills/<name>/SKILL.md`
- Configuração global: `~/.config/HanuBees/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o HanuBees sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.HanuBees/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/HanuBees/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.HanuBees/skills/git-release/SKILL.md` assim:
...
O HanuBees lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `HanuBees.json`:
...
**Para agentes embutidos** (em `HanuBees.json`):
