...
export async function handler(
      ...
      if (!allowedRegions?.includes("unavailable"))
        throw new RegionError(
          t("zen.api.error.regionNotAllowed", {
            consoleGoUrl: `https://hanbees.com/workspace/${authInfo.workspaceID}/go`,
          }),
        )
    }
    */
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://hanbees.com/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://hanbees.com/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://hanbees.com/workspace/${authInfo.workspaceID}/members`
