...
Move the canonical HanuBees terminal application from
...
The SDK is the TUI's HanuBees boundary. Missing backend data or operations must
...
- HanuBees domain data displayed by the TUI
...
route, and feature-plugin code no longer reads HanuBees globals or process state;
...
Replace process-global HanuBees state with resolved TUI inputs.
...
adapters rather than HanuBees domain implementations.

Make the SDK the only HanuBees domain boundary used by the TUI.
...
- Domain-facing TUI code imports HanuBees data and operations only from
  `@opencode-ai/sdk`.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for HanuBees domain state.
